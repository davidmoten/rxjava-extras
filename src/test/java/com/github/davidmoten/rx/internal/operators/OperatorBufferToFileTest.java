...
public final class OperatorBufferToFileTest {
	...
	public void checkRateForOneKMessagesNoRollover() {
		checkRateForOneMessagesWithOptions(Options.disableRollover().build());
	}

	@Test
	public void checkRateForOneKMessagesRollover() {
		checkRateForOneMessagesWithOptions(Options.rolloverSizeBytes(Long.MAX_VALUE - 1).build());
	}

	private static void checkRateForOneMessagesWithOptions(Options options) {
		Scheduler scheduler = Schedulers.from(Executors.newFixedThreadPool(1));
