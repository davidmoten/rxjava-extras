...
public class TransformersTest {
	...
	public void testStatisticsPairOnEmptyStream() {
		Observable<Integer> nums = Observable.empty();
		boolean isEmpty = nums.compose(Transformers.collectStats(Functions.<Integer> identity())).isEmpty().toBlocking()
				.single();
		assertTrue(isEmpty);
	}
	...
	public void testStatisticsPairOnSingleElement() {
		Observable<Integer> nums = Observable.just(1);
		Pair<Integer, Statistics> s = nums.compose(Transformers.collectStats(Functions.<Integer> identity())).last()
				.toBlocking().single();
	...
	public void testSort() {
		...
		List<Integer> list = o.compose(Transformers.<Integer> sort()).toList().toBlocking().single();
	...
	public void testDoOnNth() {
		...
		Observable.just(1, 2, 3).compose(Transformers.doOnNext(2, Actions.setAtomic(item))).subscribe();
	...
	public void testDelayUnsubscribeForRefCount() {
		...
		Observable<Long> o = Observable //
				...
				.publish().refCount() //
				.compose(Transformers.<Long> delayUnsubscribeForRefCount(2500, TimeUnit.MILLISECONDS, s));
		{
