...
class FileBasedSPSCQueue<T> implements QueueWithResources<T> {
                    ...
                    while (true) {
                        ...
                        if (over > 0) {
                            ...
                            if (index >= writeBuffer.length) {
                                ...
                                synchronized (writeLock) {
                                    writeBufferUnchanged = wp == writePosition
                                            && wbp == writeBufferPosition;
    ...
    public boolean offer(T t) {
        ...
        //offer calls must be sequential but can happen concurrently with other methods except unsubscribe
        try {
    ...
    public T poll() {
        //limited thread-safety
        //poll calls must be sequential but can happen concurrently with other methods except unsubscribe
        try {
