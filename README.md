...
----------------------------------------
As of 0.7.1-RC1, if you add a dependency for [MapDB](http://www.mapdb.org) you can offload an observable's emissions to disk to reduce memory pressure when you have a fast producer + slow consumer (or just to minimize memory usage). 
